...
#
# -	make_order()
# o	Set the attribute ordered to True and return the following message "You've ordered pizza {pizza_name} prepared with
# {ingredient: quantity} and the price will be {price}lv.". The ingredients should be separated by a comma and a space
# ", "
...
class PizzaDelivery:
    ...
    def make_order(self):
        self.ordered = True
        print_ingredients = ', '.join([f'{key}: {value}' for key, value in self.ingredients.items()])
        return f"You've ordered pizza {self.name} prepared with {print_ingredients} and the price will be {self.price}lv."
