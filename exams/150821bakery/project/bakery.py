...
class Bakery:
    ...
    @name.setter
    def name(self, value):
        Validator.raise_if_value_is_empty_string_or_white_space(value, "Name cannot be empty string or white space!")
        self.__name = value

    def add_food(self, food_type: str, name: str, price: float):
        Validator.raise_if_food_exist_in_menu(name, self.food_menu, f"{food_type} {name} is already in the menu!")
    ...
    def add_drink(self, drink_type: str, name: str, portion: float, brand: str):
        Validator.raise_if_drink_exist_in_menu(name, self.drinks_menu, f"{drink_type} {name} is already in the menu!")
        drink = DrinkFactory.create_drink(drink_type, name, portion, brand)
        self.drinks_menu.append(drink)
        return f"Added {name} ({brand}) to the drink menu"

    def add_table(self, table_type: str, table_number: int, capacity: int):
        Validator.raise_if_table_exist(
            table_number,
            self.tables_repository,
    ...
    def order_food(self, table_number: int, *food_names):
        ...
        for food_name in food_names:
            food = self.__find_instance_by_name(food_name, self.food_menu)
            if food is not None:
    ...
    @staticmethod
    def __find_instance_by_name(name, collection):
    ...
    def order_drink(self, table_number: int, *drink_names):
        ...
        for drink_name in drink_names:
            drink = Bakery.__find_instance_by_name(drink_name, self.drinks_menu)
